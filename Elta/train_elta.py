...
from model_elta import Model
from Elta_mixup import mixup, simloss
...
def train_on_gpu(config, checkpoint_folder):
    ...
    if config.training.warm_start:
        ...
        epoch = 0
    print('\nPlease be patient the first epoch is the longest one and we are preparing pos embeds :) ')
    ...
    for epoch in tqdm(range(epoch, config.training.num_epochs)):
        ...
        for index, sample in enumerate(dataloaders['train']):
            ...
            features, outputs = model(inputs, pos_embeds, masks)

            features, labels = mixup(features, labels, 0.2, 4)
            outputs = model.mlp_head.activation(model.mlp_head.linear(features))
            ...
            loss += 1 * simloss(features, labels)
        ...
        with torch.no_grad():
            for index, sample in enumerate(dataloaders['val']):
                ...
                _, outputs = model(inputs, pos_embeds, masks)
